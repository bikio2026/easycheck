...
export default function JoinByCode() {
  ...
  async function handleJoin(e) {
    ...
    setError(null);
    try {
      const { session, diner } = await api.joinByCode({
    ...
    <div className="min-h-dvh flex flex-col items-center justify-center px-4 bg-surface-alt">
      <div className="w-full max-w-sm">
        <div className="text-center mb-6">
          ...
          <p className="text-text-muted text-sm mt-1">Unirte a una mesa</p>
        </div>

        <form onSubmit={handleJoin} className="bg-white rounded-2xl shadow-sm border border-border p-6 space-y-5">
          <div>
            <label className="block text-sm font-medium mb-2">Codigo de invitacion</label>
            <input
              type="text"
              value={code}
              ...
              placeholder="ABC123"
              maxLength={6}
              className="w-full px-4 py-3 border border-border rounded-xl text-center text-lg font-mono tracking-widest uppercase focus:outline-none focus:ring-2 focus:ring-primary/40"
            />
          </div>
